...
```php
static $pdo = null;   // static = 関数が終わっても値が残る
if ($pdo === null) {  // 初回だけ接続処理を実行
    $pdo = new PDO(...);
}
...
```
1回目の呼び出し → $pdo が null → 新規接続 → $pdo に保存
2回目以降      → $pdo に値がある → 接続せずそのまま return
```
...
| オプション | 意味 |
|-----------|------|
| `ERRMODE_EXCEPTION` | SQLエラーを例外として検知する（デフォルトは無視するため必須） |
| `FETCH_ASSOC` | 取得結果を `$row['id']` のような連想配列で受け取る |
| `EMULATE_PREPARES => false` | 本物のプリペアドステートメントを使う（SQLインジェクション対策） |

**⑤ try〜catch**

接続失敗時（DB未起動・パスワード違いなど）に `PDOException` を捕捉してエラー表示で停止する。
